...
import { LayoutGrid, BarChart3, Sun, Moon, Bell, Search, Command, Cpu, Sparkles } from 'lucide-react'
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', overflow: 'hidden', background: c.bg, color: c.text, fontFamily: 'Onest, Inter, system-ui, sans-serif' }}>
      <aside style={{
        ...
        {/* Centered logo */}
        <div style={{ padding: '20px 16px 16px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10, borderBottom: `1px solid ${c.border}` }}>
          <Logo size={44} />
        ...
        <nav style={{ flex: 1, padding: '12px 10px', display: 'flex', flexDirection: 'column', gap: 2, overflowY: 'auto' }}>
          ...
          <div style={{ marginTop: 18 }}>
            ...
            {MOCK_CASES.filter(c => ['critical', 'high'].includes(c.urgency) && c.status !== 'filed').slice(0, 6).map(cs => {
              ...
              return (
                <NavLink key={cs.id} to={`/case/${cs.id}`} style={{
                  display: 'flex', alignItems: 'center', gap: 9,
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0 }}>
        <header style={{
          ...
          <div style={{
            ...
            <input placeholder="Search cases, documents, agents..." style={{
              background: 'transparent', border: 'none', outline: 'none',
              color: c.text, fontSize: 12.5, flex: 1, fontFamily: 'inherit',
            }} />
            <span style={{
